        ...
        <View style={styles.searchContainer}>
            ...
            <FlatList
                ...
                renderItem={(item) => {
                    return (
                        <SearchItem
                            idMeal={item.item.idMeal}
                            strArea={item.item.strArea}
                            strCategory={item.item.strCategory}
                            strInstructions={item.item.strInstructions}
                            strMeal={item.item.strMeal}
                            strMealThumb={item.item.strMealThumb}
                            strTags={item.item.strTags ?? ""}
                            strDrinkAlternate={item.item.strDrinkAlternate ?? ""}
                        />
                    );
                }}
            />
...
function SearchItem(props: SearchItemProps) {
    let tags = "";
    if (props.strTags) {
        const aux = props.strTags.includes(",");
        tags = aux ? props.strTags.substring(0, props.strTags.indexOf(",")) : props.strTags;
    }
    return (
                ...
                <View style={styles.itemTestTagContainer}>
                    {tags && (
                        <>
                            ...
                            <Text style={styles.itemTextTag}>{props.strTags}</Text>
                        </>
                    )}
                </View>
                <Text style={styles.itemTextTitle}>{props.strMeal}</Text>
                <Text style={styles.itemTextArea}>{props.strArea}</Text>
            </View>
            <Image style={styles.image} src={props.strMealThumb} />
        </TouchableOpacity>
    );
}
